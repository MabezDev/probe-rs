...
/// The `DebuggerStatus` is used to control how the Debugger::debug_session() decides if it should
/// respond to DAP Client requests such as `Terminate`, `Disconnect`, and `Reset`, as well as how to
/// respond to unrecoverable errors during a debug session interacting with a target session.
